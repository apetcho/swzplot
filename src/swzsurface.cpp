...
Surface SurfaceBase::surf(
    ...
    this->m_cdata.clear();
    this->m_edgeColor = "k";
    this->m_faceColor = "flat";

    return this->share();
}

// -*-
Surface SurfaceBase::pcolor(const Matrix<double>& cmat){
    std::unique_lock<std::mutex> lock(this->m_data_mtx);
...
// -*-
Surface SurfaceBase::contour(const Matrix<double>& zmat, const Vector<double>& values){
    std::unique_lock<std::mutex> lock(this->m_data_mtx);
    this->m_axType = AxesType::ContourPlot;
    this->m_xdata.clear();
    this->m_ydata.clear();
    this->m_zdata = zmat;
    this->m_numContour = values.size();
    this->m_vdata = values;
    return this->share();
}
